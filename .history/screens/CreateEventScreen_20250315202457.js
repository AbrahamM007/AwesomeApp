  ...
  const handleCreateEvent = async () => {
    ...
    try {
      ...
      try {
        ...
        // Use a shorter timeout since we already saved locally
        ...
        if (docRef) {
          ...
          // Update the local copy with the Firestore ID
          const updatedEvents = events.map(event => 
            event.id === localEventData.id ? {...event, id: docRef.id} : event
          );
          ...
          if (isPublic) {
            try {
              ...
              // Save announcement to Firestore with a short timeout
              const announcementPromise = addDoc(collection(db, 'announcements'), announcementData);
              const announcementTimeoutPromise = new Promise((_, reject) => 
                setTimeout(() => reject(new Error('Announcement timeout')), 3000)
              );
              
              const announcementRef = await Promise.race([
                announcementPromise, 
                announcementTimeoutPromise
              ]);
              ...
              announcements.push({
                ...announcementData,
                id: announcementRef ? announcementRef.id : `local-${Date.now()}-announcement`,

...
const CreateEventScreen = () => {
  ...
  const handleCreateEvent = async () => {
    ...
    try {
      ...
      // Check network connectivity using expo-network
...
const handleCreateEvent = async () => {
  ...
  try {
    ...
    // Create a unique ID for the event that we'll use for both local and Firestore
    ...
    console.log('Event saved locally with ID:', eventId);
    ...
    if (isConnected) {
      try {
        console.log('Network available, attempting to save to Firestore');
        ...
        // Use a shorter timeout and try to use setDoc with a predefined ID instead of addDoc
        const timeoutPromise = new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Firestore connection timeout')), 5000)
        );
        
        // Try to use setDoc with our predefined ID instead of letting Firestore generate one
        await Promise.race([
          setDoc(doc(db, 'events', eventId), firestoreData),
          timeoutPromise
        ]);
        
        console.log('Event saved to Firestore with ID:', eventId);
        firestoreSuccess = true;
        
        // Update local storage to mark as synced
        const updatedEvents = events.map(event => 
          event.id === eventId ? {...event, pendingSync: false} : event
        );
        await AsyncStorage.setItem('events', JSON.stringify(updatedEvents));
        ...
        if (isPublic) {
          try {
            const announcementId = `announcement_${Date.now()}_${Math.floor(Math.random() * 1000)}`;
            const announcementData = {
              title: eventName,
              description: description,
              imageUrl: eventImage,
            ...
            // Use setDoc for announcement too
            await Promise.race([
              setDoc(doc(db, 'announcements', announcementId), announcementData),
              new Promise((_, reject) => setTimeout(() => reject(new Error('Announcement timeout')), 3000))
            ]);
        ...
        console.error('Error saving to Firestore:', firestoreError);
        // Already marked as pendingSync: true, so no need to update
      }
    } else {
      console.log('No network connection, saved locally only');
    }

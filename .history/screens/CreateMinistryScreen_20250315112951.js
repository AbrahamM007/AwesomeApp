...
const CreateMinistryScreen = () => {
  ...
  const handleCreateMinistry = async () => {
    ...
    try {
      ...
      // Save to Firestore
      const ministryRef = await firestore().collection('ministries').add(ministryData);
      
      // If public, also add to announcements collection
      if (isPublic) {
        await firestore().collection('announcements').add({
          ...
          imageUrl: ministryImage,
          type: 'ministry',
          ministryId: ministryRef.id, // Reference to the original ministry
          createdAt: new Date(),
          createdBy: user?.uid || 'anonymous'
        });
      }

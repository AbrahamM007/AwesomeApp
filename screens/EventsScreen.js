...
import { collection, query, orderBy, getDocs, onSnapshot, enableNetwork, disableNetwork, connectFirestoreEmulator } from 'firebase/firestore';
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      try {
        ...
        // Get events from Firebase using v9 syntax
        ...
        try {
          ...
          console.log('Network already enabled or error:', networkErr);
        }
    ...
    // Set up real-time listener for events collection using v9 syntax
    const eventsRef = collection(db, 'events');
    const q = query(eventsRef, orderBy('date', 'desc'));
    
    // Changed variable name to avoid conflict with the one declared at the top

...
const CreateEventScreen = () => {
  ...
  const handleCreateEvent = async () => {
    ...
    try {
      ...
      let networkState;
      try {
        networkState = await Promise.race([
      ...
      if (isConnected) {
        try {
          ...
          // Try to use setDoc with our predefined ID
          await Promise.race([
          ...
          if (isPublic) {
            try {
              ...
              // Use setDoc for announcement with a shorter timeout
              await Promise.race([
                setDoc(doc(db, 'announcements', announcementId), announcementData),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Announcement timeout')), 2000))
              ]);
